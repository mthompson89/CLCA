CLCA_bystand <- function(Cutlist, CarbE = TRUE,Treelist = FALSE, Carbonsummary, per_energy, per_ef, Region1 ){
  ...
  if(Treelist == FALSE){
    ...
    loop_rate_table_stand <- data.frame(year = c(1:41),
                                        SWSL_inuse = integer(41),
                                        SWPW_inuse = integer(41),
                                        HWSL_inuse = integer(41),
                                        HWPW_inuse = integer(41),
                                        SWSL_Landfill = integer(41),
                                        SWPW_Landfill = integer(41),
                                        HWSL_Landfill = integer(41),
                                        HWPW_Landfill = integer(41)
    )  
    
    #creating loop_rate Table for loop
    for (i in 1:41){
      loop_rate_table_stand$SWSL_inuse[i] <- inuse_rate_func(i-1,"Softwood","Saw log", Region1)
      loop_rate_table_stand$SWPW_inuse[i] <- inuse_rate_func(i-1,"Softwood","Pulpwood", Region1)
      loop_rate_table_stand$HWSL_inuse[i] <- inuse_rate_func(i-1,"Hardwood","Saw log", Region1)
      loop_rate_table_stand$HWPW_inuse[i] <- inuse_rate_func(i-1,"Hardwood","Pulpwood", Region1)
      
      
      loop_rate_table_stand$SWSL_Landfill[i] <- landfill_rate_func(i-1,"Softwood","Saw log", Region1)
      loop_rate_table_stand$SWPW_Landfill[i] <- landfill_rate_func(i-1,"Softwood","Pulpwood", Region1)
      loop_rate_table_stand$HWSL_Landfill[i] <- landfill_rate_func(i-1,"Hardwood","Saw log", Region1)
      loop_rate_table_stand$HWPW_Landfill[i] <- landfill_rate_func(i-1,"Hardwood","Pulpwood", Region1)
      
    }
    
    
    #shift rate table to allow for year -1
    loop_rate_table_stand <- loop_rate_table_stand%>%
    ...
    CO2_OT_bystand <- data.frame(year = rep(0:40, times = numStands),
                                 StandID = rep(DataByStand$StandID, each = 41),
                                 in_use = numeric(41*nrow(DataByStand)),
                                 landfill = numeric(41*nrow(DataByStand)))
