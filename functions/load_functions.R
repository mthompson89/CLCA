...
cppFunction(
  'DataFrame rcppForLoop_lapply(DataFrame CO2, DataFrame Data, DataFrame Loop) {
...
NumericVector LRT_HWPW_Landfill = Loop["HWPW_Landfill"];
  ...
  //and increase stand by 1 (stand++)
      ...
      for (int i = 0; i < 46; i++){
        ...
        CO2U[i] = (DBS_SWSL_MgC[i] * LRT_SWSL_inuse[i])    + (DBS_SWPW_MgC[i] * LRT_SWPW_inuse[i]) +
                      (DBS_HWSL_MgC[i] * LRT_HWSL_inuse[i])    + (DBS_HWPW_MgC[i] * LRT_HWPW_inuse[i]);

        CO2L[i] = (DBS_SWSL_MgC[i] * LRT_SWSL_Landfill[i]) + (DBS_SWPW_MgC[i] * LRT_SWPW_Landfill[i]) +
                      (DBS_HWSL_MgC[i] * LRT_HWSL_Landfill[i]) + (DBS_HWPW_MgC[i] * LRT_HWPW_Landfill[i]);
      }
...
cppFunction(
  'DataFrame rcppForLoopStand(DataFrame CO2, DataFrame Data, DataFrame Loop) {
...
int x = 4280;
  ...
  for (int i = 0; i < x; i++){
      ...
      for (int j = 0; j < 41; j++){
        ...
        CO2U[i*41+j] = (DBS_SWSL_MgC[i] * LRT_SWSL_inuse[j])    + (DBS_SWPW_MgC[i] * LRT_SWPW_inuse[j]) +
                      (DBS_HWSL_MgC[i] * LRT_HWSL_inuse[j])    + (DBS_HWPW_MgC[i] * LRT_HWPW_inuse[j]);

        CO2L[i*41+j] = (DBS_SWSL_MgC[i] * LRT_SWSL_Landfill[j]) + (DBS_SWPW_MgC[i] * LRT_SWPW_Landfill[j]) +
                      (DBS_HWSL_MgC[i] * LRT_HWSL_Landfill[j]) + (DBS_HWPW_MgC[i] * LRT_HWPW_Landfill[j]);
      }
  }
